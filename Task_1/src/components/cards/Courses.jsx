...
const Courses = () => (
    <div>
      <div className='wrapper flex'>
           <div><Input id = "input" onChange={(event => {inputValue = event.target.value})} ></Input><Button name="Search" className="classButtonHeader"
           onClick = {() => mockedCourseList.filter(course => {
            return course.title.toLowerCase().includes(inputValue.toLowerCase())
          })}></Button></div>
           <div><Button name="Add new course" className="newCourse" onClick = {() => {}} ></Button></div>
        </div>
      {mockedCourseList.map(item => (
         <div className='wrapper'><Card title = {item.title} 
                                        ...
                                        authors = {mockedAuthorsList.filter(author => item.authors.includes(author.id)).map((author) => {
                                                                                                                                    return author.name; 
                                                                                                                                        })}>
                                    </Card></div>
      ))}
